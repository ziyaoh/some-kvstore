...
// RaftRPCClientConn creates or returns a cached RPC client for the given remote node
func (remote *RemoteNode) RaftRPCClientConn() (RaftRPCClient, error) {
	connMapLock.RLock()
...
func (remote *RemoteNode) JoinRPC(local *RemoteNode) error {
	...
	ok, err := cc.JoinCaller(context.Background(), local)
	if err == nil && !ok.GetOk() {
